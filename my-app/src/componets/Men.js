...
function Men() {
    ...
    let currentpds=data.slice(firstpdIndex,lastpdIndex)
    const pages=[];
    ...
    <input type="text" class="input-search" onChange={(e)=>{searchhandle(e.target.value)}} placeholder="Type to Search..."/>
            ...
            {
                detail === 1 && <>
                <Button className='search_back' onClick={(e)=>{window.location.reload();}} variant="outline-secondary"><CloseIcon/></Button>
                </>
            }
